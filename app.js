document.addEventListener('DOMContentLoaded', function() {
    ...
    `;
        ...
        const TILE_COUNT = 180;
        const heartLayout = computeHeartLayout(TILE_COUNT);
        const blessings = generateBlessings(TILE_COUNT);
        const colors = generateTileColors(TILE_COUNT);
        ...
        setHeartContainerSize(container);
        positionTilesAsHeart(tiles, container, heartLayout, { animateFromScatter: true }, () => {
            assembled = true;
        ...
        const resizeHandler = () => {
            if (!flattened) {
                setHeartContainerSize(container);
                positionTilesAsHeart(tiles, container, heartLayout, { instant: true });
            } else {
    ...
    function setHeartContainerSize(container) {
        const width = Math.max(420, Math.min(window.innerWidth * 0.75, 700));
        const height = Math.max(420, Math.min(window.innerHeight * 0.7, 620));
    ...
    function positionTilesAsHeart(tiles, container, layout, options = {}, onComplete) {
        ...
        const baseDelay = animateFromScatter ? 240 : 20;
        const stepDelay = instant ? 0 : 45;
